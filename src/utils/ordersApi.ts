...
export const LAST_ORDER_EMAIL_KEY = 'lb_last_order_email';
...
export async function fetchOrdersFromServer(email: string): Promise<{
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/api/orders?email=${q}`, {
...
export async function fetchOrderTimeline(orderId: string): Promise<{
  ...
  try {
    const res = await fetch(`${API_BASE}/api/orders/${encodeURIComponent(orderId)}/timeline`, {
      headers: orderAccessHeaders(),
    });
    const data = await res.json();
...
export async function enqueuePaymentVerification(input: {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/payments/verify-queue`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
